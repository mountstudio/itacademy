...
abstract class Config implements ActiveRecordInterface
{
    ...
    /**
     * The unserialized $seo value - i.e. the persisted object.
     * This is necessary to avoid repeated calls to unserialize() at runtime.
     * @var object
     */
    protected $seo_unserialized;
    ...
    /**
     * Get the [seo] column value.
     *
     * @return \Core\Seo
     */
    public function getSeo()
    {
        if (null == $this->seo_unserialized && is_resource($this->seo)) {
            if ($serialisedString = stream_get_contents($this->seo)) {
                $this->seo_unserialized = unserialize($serialisedString);
            }
        }

        return $this->seo_unserialized;
    }
    ...
    public function setData($v)
    {
        ...
        if ($this->data !== $v) {
            $this->data = $v;
            $this->modifiedColumns[ConfigTableMap::COL_DATA] = true;
        }
    ...
    /**
     * Set the value of [seo] column.
     *
     * @param \Core\Seo $v new value
     * @return $this|\Models\Config The current object (for fluent API support)
     */
    public function setSeo($v)
    {
        if (null === $this->seo || stream_get_contents($this->seo) !== serialize($v)) {
            $this->seo_unserialized = $v;
            $this->seo = fopen('php://memory', 'r+');
            fwrite($this->seo, serialize($v));
            $this->modifiedColumns[ConfigTableMap::COL_SEO] = true;
        }
        rewind($this->seo);
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ConfigTableMap::translateFieldName('Data', TableMap::TYPE_PHPNAME, $indexType)];
            $this->data = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ConfigTableMap::translateFieldName('Seo', TableMap::TYPE_PHPNAME, $indexType)];
            if (null !== $col) {
                $this->seo = fopen('php://memory', 'r+');
                fwrite($this->seo, $col);
                rewind($this->seo);
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->isNew() || $this->isModified()) {
                ...
                // Rewind the seo LOB column, since PDO does not rewind after inserting value.
                if ($this->seo !== null && is_resource($this->seo)) {
                    rewind($this->seo);
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ConfigTableMap::COL_DATA)) {
            $modifiedColumns[':p' . $index++]  = 'data';
        }
        if ($this->isColumnModified(ConfigTableMap::COL_SEO)) {
            $modifiedColumns[':p' . $index++]  = 'seo';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'data':
                        ...
                        break;
                    case 'seo':
                        if (is_resource($this->seo)) {
                            rewind($this->seo);
                        }
                        $stmt->bindValue($identifier, $this->seo, PDO::PARAM_LOB);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ConfigTableMap::COL_DATA)) {
            $criteria->add(ConfigTableMap::COL_DATA, $this->data);
        }
        if ($this->isColumnModified(ConfigTableMap::COL_SEO)) {
            $criteria->add(ConfigTableMap::COL_SEO, $this->seo);
        }
