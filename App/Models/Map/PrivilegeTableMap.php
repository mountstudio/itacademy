...
class PrivilegeTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Models.Map.PrivilegeTableMap';
     ...
     */
    const OM_CLASS = '\\Models\\Privilege';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Models.Privilege';
     ...
     */
    const COL_ID = 'privilege.id';

    /**
     * the column name for the name field
     */
    const COL_NAME = 'privilege.name';

    /**
     * the column name for the alt field
     */
    const COL_ALT = 'privilege.alt';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Name', 'Alt', ),
        self::TYPE_CAMELNAME     => array('id', 'name', 'alt', ),
        self::TYPE_COLNAME       => array(PrivilegeTableMap::COL_ID, PrivilegeTableMap::COL_NAME, PrivilegeTableMap::COL_ALT, ),
        self::TYPE_FIELDNAME     => array('id', 'name', 'alt', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Name' => 1, 'Alt' => 2, ),
        self::TYPE_CAMELNAME     => array('id' => 0, 'name' => 1, 'alt' => 2, ),
        self::TYPE_COLNAME       => array(PrivilegeTableMap::COL_ID => 0, PrivilegeTableMap::COL_NAME => 1, PrivilegeTableMap::COL_ALT => 2, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'name' => 1, 'alt' => 2, ),
        self::TYPE_NUM           => array(0, 1, 2, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\Models\\Privilege');
        $this->setPackage('Models');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('name', 'Name', 'VARCHAR', true, 255, null);
        $this->addColumn('alt', 'Alt', 'VARCHAR', true, 45, null);
    } // initialize()
    ...
    {
        $this->addRelation('CurrentPrivilegeGroupPrivelege', '\\Models\\GroupPrivelege', RelationMap::ONE_TO_MANY, array (
  0 =>
  array (
    0 => ':privilege_id',
    1 => ':id',
  ),
), 'CASCADE', null, 'CurrentPrivilegeGroupPriveleges', false);
        $this->addRelation('CurrentGroupGroupPrivelege', '\\Models\\Group', RelationMap::MANY_TO_MANY, array(), 'CASCADE', null, 'CurrentGroupGroupPriveleges');
    } // buildRelations()
    ...
    /**
     * Method to invalidate the instance pool of all tables related to privilege     * by a foreign key with ON DELETE CASCADE
     */
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(PrivilegeTableMap::COL_ID);
            $criteria->addSelectColumn(PrivilegeTableMap::COL_NAME);
            $criteria->addSelectColumn(PrivilegeTableMap::COL_ALT);
        } else {
    ...
    public static function buildTableMap()
    {
        ...
        if (!$dbMap->hasTable(PrivilegeTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PrivilegeTableMap());
        }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Models\Privilege) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(PrivilegeTableMap::DATABASE_NAME);
            $criteria->add(PrivilegeTableMap::COL_ID, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(PrivilegeTableMap::COL_ID) && $criteria->keyContainsValue(PrivilegeTableMap::COL_ID) ) {
...
//
PrivilegeTableMap::buildTableMap();
