...
/**
 * This class defines the structure of the 'passport' table.
 *
 *
 *
 ...
 *
 */
class PassportTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Models.Map.PassportTableMap';
     ...
     */
    const OM_CLASS = '\\Models\\Passport';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Models.Passport';
     ...
     */
    const COL_SERIAL = 'passport.serial';

    /**
     * the column name for the inn field
     */
    const COL_INN = 'passport.inn';

    /**
     * the column name for the start_date field
     */
    const COL_START_DATE = 'passport.start_date';

    /**
     * the column name for the operator field
     */
    const COL_OPERATOR = 'passport.operator';

    /**
     * the column name for the user_id field
     */
    const COL_USER_ID = 'passport.user_id';
  ...
  array (
    ...
    {
        return $withPrefix ? PassportTableMap::CLASS_DEFAULT : PassportTableMap::OM_CLASS;
    }
     ...
     * @return array           (Passport object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PassportTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PassportTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PassportTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PassportTableMap::OM_CLASS;
            /** @var Passport $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PassportTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PassportTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PassportTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PassportTableMap::DATABASE_NAME)->getTable(PassportTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PassportTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PassportTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PassportTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Passport or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Passport object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Models\Passport) { // it's a model object
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                PassportTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PassportQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Passport or Criteria object.
     *
     * @param mixed               $criteria Criteria or Passport object containing data that is used to create the INSERT statement.
...
//
PassportTableMap::buildTableMap();
