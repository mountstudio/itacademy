...
class User extends BaseUser
{
  public function getLogo()
  {
    ...
    return array(   'thumb'     => $validLogoUploadsDirName . Config::USER_LOGO_THUMBNAIL_DIR_NAME . "/" . $logoName,
                    'normal'     => $validLogoUploadsDirName . Config::USER_LOGO_NORMAL_DIR_NAME . "/" . $logoName,
                    'original'  => $validLogoUploadsDirName . Config::USER_LOGO_ORIGINAL_DIR_NAME . "/" . $logoName);
  }

  public function getTeachingCourses(){
      $courses = array();
      foreach ($this->getCurrentInstructorCourseStreamsJoinCurrentCourseCourseStream() as $course){
          $course = $course->getCurrentCourseCourseStream();
          if (!array_key_exists($course->getId(), $courses)){
  ...
  public function getCover()
  {
    ...
    return array(   'normal'     => $validCoverUploadsDirName . Config::USER_COVER_NORMAL_DIR_NAME . "/" . $coverName,

                    'original'  => $validCoverUploadsDirName . Config::USER_COVER_ORIGINAL_DIR_NAME . "/" . $coverName);
  }
  ...
  public function setLogo($file)
  {
    ...
    public function convertCurrency(float $value, Currency $currency = null)
    {
        ...
        $activeCurrency = $defaultShownCurrency;
        ...
        if ($activeCurrency->getId() != $currency->getId())
        {
            if ($defaultCurrency->getId() != $currency->getId()){
                $currencyRate_ = CurrencyRateQuery::create()->filterByCurrentDefaultCurrency($defaultCurrency)->filterByCurrentToCurrency($currency)->orderByCreatedAt(Criteria::DESC)->findOne();
                $currentValue_ = $currencyRate_->getRate();
                $value *= $currentValue_;
            }

            if ($activeCurrency->getId() != $defaultCurrency->getId()){
                $currencyRate = CurrencyRateQuery::create()->filterByCurrentDefaultCurrency($defaultCurrency)->filterByCurrentToCurrency($activeCurrency)->orderByCreatedAt(Criteria::DESC)->findOne();
